...
(async () => {
    if (process.version.slice(1).split(".")[0] < 16) {
        ...
        logger.error(
            "CHL requires Node 16 or higher. Please update your version of Node."
        );
        process.exit(0);
    }
...
async function checkUpdate() {
    ...
    try {
        const res = await axios.get(
            "https://raw.githubusercontent.com/Chaelllllllll/chl-help/main/package.json"
        );
...
async function main() {
    ...
    child.on("close", async (code) => {
        ...
        if (code !== 0 && restartCount < 5) {
            ...
            console.log();
            logger.error("CHL has stopped, press Ctrl + C to exit.");
        }
    });
}
